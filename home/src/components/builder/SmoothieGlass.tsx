...
import { blendLiquid, lighten, darken, FRUITS } from './builderData'

interface SmoothieGlassProps {
  state: BuilderState
  variant?: 'lg' | 'md' | 'sm'
}
...
export default function SmoothieGlass({ state, variant = 'lg' }: SmoothieGlassProps) {
  const color = blendLiquid(state)
  const top  = lighten(color, 0.20)
  const mid  = color
  const bot  = darken(color, 0.18)
  const foam = lighten(color, 0.40)
  const empty = state.fruits.length === 0 && !state.boosts.some(b => ['pb','espresso','oats'].includes(b))
  const fill = 0.90
    ...
    <div style={{ width: dims.w, height: dims.h, position: 'relative' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          height: `${fill * 100}%`,
          background: `radial-gradient(120% 60% at 30% 8%, ${lighten(top, 0.12)} 0%, transparent 45%), linear-gradient(180deg, ${top} 0%, ${mid} 42%, ${bot} 100%)`,
        }}>
          {/* speckle */}
          <div style={{
            position: 'absolute', inset: 0, opacity: empty ? 0.10 : 0.22, mixBlendMode: 'multiply',
            background: `radial-gradient(1.6px 1.6px at 20% 30%, ${darken(mid,0.3)} 99%, transparent), radial-gradient(1.4px 1.4px at 60% 50%, ${darken(mid,0.35)} 99%, transparent), radial-gradient(1.5px 1.5px at 80% 25%, ${darken(mid,0.3)} 99%, transparent), radial-gradient(1.3px 1.3px at 40% 70%, ${darken(mid,0.35)} 99%, transparent), radial-gradient(1.6px 1.6px at 75% 80%, ${darken(mid,0.3)} 99%, transparent)`,
            backgroundSize: '70px 70px',
          }} />
          {/* left sheen */}
          <div style={{ position: 'absolute', top: 0, left: 0, width: '26%', height: '100%', background: 'linear-gradient(90deg, rgba(255,255,255,.22), transparent)' }} />
        </div>
        ...
        {variant !== 'sm' && bits.map((c, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              boxShadow: '0 1px 3px rgba(0,0,0,.3)', zIndex: 4,
            }} />
          )
        })}

        {/* highlight */}
        <div style={{ position: 'absolute', top: '4%', left: '10%', width: '8%', height: '78%', borderRadius: 20, background: 'linear-gradient(180deg, rgba(255,255,255,.5), rgba(255,255,255,0))', opacity: 0.5, zIndex: 6 }} />
        {/* rim shadow */}
        <div style={{ position: 'absolute', top: 0, right: 0, width: '18%', height: '100%', background: 'linear-gradient(270deg, rgba(0,0,0,.18), transparent)', zIndex: 6 }} />
      </div>

      {/* base shadow */}
      <div style={{ position: 'absolute', left: '50%', bottom: -dims.h * 0.03, transform: 'translateX(-50%)', width: dims.w * 0.92, height: dims.h * 0.05, borderRadius: '50%', background: 'radial-gradient(closest-side, rgba(0,0,0,.5), transparent)', filter: 'blur(3px)', zIndex: 0 }} />
    </div>
  )
}
